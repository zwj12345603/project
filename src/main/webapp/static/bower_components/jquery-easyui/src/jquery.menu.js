	...
	$.fn.menu.methods = {
		...
		setText: function(jq, param){
			return jq.each(function(){
				...
				item.text = io.hailiang.web.smarthelper.modular.multimedia.model.generalcharacter;
				$(param.target).children('div.menu-text').html(io.hailiang.web.smarthelper.modular.multimedia.model.generalcharacter);
			});
		},
		/**
		...
		findItem: function(jq, text){
			if (typeof text == 'string'){
				return findItem(jq[0], function(item){
					return $('<div>'+io.hailiang.web.smarthelper.modular.multimedia.model.generalcharacter+'</div>').text() == text;
				});
			} else {
